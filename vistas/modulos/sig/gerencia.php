          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header bg-info">
                <h3 class="card-title">ACPM GERENCIA</h3>
              </div>
              <div class="card-body">
                <table class="display table table-bordered" width="100%">
                  <thead class="bg-dark">
                   ...
                   <tbody>
                        ...
                        $item = null;
                        $valor = null;
                        $acpm_sig = ControladorAcpm::ctrMostrarAcpmGerencia($item,$valor);
                        foreach ($acpm_sig as $key => $value) {
                             $informe_acpm = "<a target='_blank' href='extensiones/tcpdf/pdf/acpmpdf.php?id={$value["id_consecutivo"]}' class='btn btn-outline-success'><i class='fas fa-file-signature'></i> Formato</a>";
                              $fecha = "<button type='button' class='btn btn-outline-info' data-toggle='modal' data-target='#modal-modificar-gerencia' data-gerencia='{$value["id_consecutivo"]}'>Modificar</button>";
                            echo "<tr>
