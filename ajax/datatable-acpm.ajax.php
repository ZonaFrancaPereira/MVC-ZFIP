...
class TablaAcpm {
    ...
    private function mostrarTabla($item, $valor, $consulta) {
        ...
        foreach ($acpm as $s) {
            if ($consulta === 'aprobar') {
                $rechazar = "<button type='button' class='btn btn-outline-info' data-id='{$s["id_consecutivo"]}'>Rechazar</button>";
                $aprobar = "<button type='button' class='btn btn-outline-info aprobarAcpm' data-id='{$s["id_consecutivo"]}'>Aprobar</button>";
